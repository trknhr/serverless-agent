export const customToolDefinitions = [
  ...
  },
  {
    ...
    description:
      "Create a recurring Slack notification generated by the assistant, such as a daily morning task reminder. Use this only when the user explicitly asks for an individual reminder, a separate reminder, or a new notification schedule. Do not use it for one-off events, packing lists, or details that should appear inside an existing daily reminder; use upsert_task, upsert_recurring_task, or save_memory for those contents.",
    input_schema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Update a recurring Slack notification generated by the assistant, including its name, prompt, output channel, enabled state, or delivery schedule. Use this only for explicit changes to a separate notification schedule. Do not use this to add one-off event details to a daily reminder; store those details with task, recurring task, or memory tools.",
    input_schema: {
      type: "object",
      properties: {
        scheduled_task_id: { type: "string" },
