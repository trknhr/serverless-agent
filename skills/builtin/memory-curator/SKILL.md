...
2. Save one memory per stable fact, preference, rule, or project detail. In interactive chat, save only after a direct remember request or approval; set `origin=inferred` explicitly for proactive candidates.
...
7. Before correcting or enriching an existing fact, call `search_context`, then call `save_memory` with the returned `memory_id` and `expected_updated_at=<returned updated_at>`.
8. For a new channel fact, provide a stable one-fact `dedupe_key`, such as `person:hanako:birthday`.
9. For birthdays, set `attributes.date_kind=birthday`; create a yearly recurring task only when the user also asks for a birthday reminder.
10. Keep memory text concise and avoid mixing unrelated facts.
...
- Never create a second memory when an existing memory can be updated.
